...
export function users(selectedUsername) {
    ...
    fetch(`/api/another_user_profile/?username=${encodeURIComponent(selectedUsername)}`, {
        method: "GET",
        credentials: "include"
    })
    .then(response => {
    ...
    .then(userData => {
        ...
        if (fullName) fullName.textContent = `${userData.first_name} ${userData.last_name}`;
        if (userName) userName.textContent = userData.user_name;
        if (ppp_rating) ppp_rating.textContent = userData.ppp_rating;
        if (player_title) player_title.textContent = userData.title;
        if (win_ratio) win_ratio.textContent = userData.win_ratio;
        if (matches_played) matches_played.textContent = userData.matches_played;
        if (profile_pic) {
            let image_path;
    ...
    return () => {
        console.log('Cleaned up users page');
    };
}